...
class QualityValidator:
    ...
    def calculate_coherence_score(self, content: str, section_type: str) -> float:
        ...
        # Step 1: Check paragraph structure
        ...
        # Step 2: Count logical connectors
        logical_connectors = [
            'therefore', 'however', 'furthermore', 'consequently',
            'moreover', 'nevertheless', 'additionally', 'meanwhile',
            'accordingly', 'otherwise', 'hence', 'thus', 'nonetheless'
        ]
        connector_count = sum(1 for word in logical_connectors if word in content_lower)
        if connector_count >= 3:
            score += 0.2
        elif connector_count >= 1:
            score += 0.1

        # Step 3: Check for structured thinking markers
        structure_markers = [
            'first', 'second', 'third', 'finally',
            'firstly', 'secondly', 'thirdly',
            'in addition', 'in conclusion', 'to begin',
            'next', 'then', 'lastly'
        ]
        ...
        # Step 4: Measure content depth (sentence count)
        sentences = [sentence.strip() for sentence in content.split('.') if len(sentence.strip()) > 5]
        if len(sentences) >= 8:
            score += 0.3
    ...
    def calculate_groundedness_score(
        ...
        # Step 1: Define section-specific keywords dictionary
        section_keywords = {
            "liability_assessment": [
                "liability", "negligence", "breach", "duty", "causation",
                "infringement", "fault", "obligation", "damages"
            ],
            "damage_calculation": [
                "damages", "compensation", "calculation", "quantum",
                "lost profits", "revenue", "valuation", "monetary"
            ],
            "prior_art_analysis": [
                "prior art", "patent", "novelty", "obviousness",
                "claims", "disclosure", "invalidity", "anticipation"
            ],
            "competitive_landscape": [
                "competitors", "market share", "positioning",
                "competitive", "market", "industry", "landscape"
            ],
            "risk_assessment": [
                "risk", "probability", "impact", "mitigation",
                "likelihood", "severity", "exposure", "contingency"
            ],
            "strategic_recommendations": [
                "recommendation", "recommend", "strategy", "implementation",
                "action plan", "timeline", "objective", "roadmap"
            ]
        }

        # Step 2: Get keywords for this section_type
        keywords = section_keywords.get(section_type, ["analysis", "assessment"])

        # Step 3: Calculate keyword coverage (up to 0.4)
        keyword_matches = sum(1 for keyword in keywords if keyword in content_lower)
        keyword_ratio = keyword_matches / len(keywords) if keywords else 0
        score += keyword_ratio * 0.4

        # Step 4: Check for reasoning indicators (up to 0.3)
        reasoning_indicators = [
            "based on", "because", "due to", "as a result",
            "according to", "therefore", "consequently",
            "this suggests", "this indicates", "evidence shows",
            "given that", "leading to", "resulting in"
        ]
        reasoning_matches = sum(1 for indicator in reasoning_indicators if indicator in content_lower)
        reasoning_ratio = reasoning_matches / len(reasoning_indicators)
        if reasoning_ratio >= 0.3:
            score += 0.3
        elif reasoning_ratio >= 0.15:
            score += 0.2
        elif reasoning_matches >= 1:
            score += 0.1

        # Step 5: Check expected elements coverage (up to 0.3)
        if expected_elements:
            element_matches = sum(1 for elem in expected_elements if elem.lower() in content_lower)
            element_ratio = element_matches / len(expected_elements)
            score += element_ratio * 0.3

        # Step 6: Calculate final score (cap at 1.0)
        return min(score, 1.0)
