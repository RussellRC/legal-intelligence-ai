...
from ..core.quality_validator import QualityValidator, QualityScore
    ...
    def generate_section_content(
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                logger.debug(f"Attempt {attempt}: Content generated in {execution_time:.2f} seconds")
                ...
                print(f"Failed to generate content. Attempt {attempt}. Retrying in {sleep_duration} seconds...")
        ...
        for section_type, persona_type in section_config:
            ...
            while attempt <= max_attempts:
                if attempt == 1:
                    ...
                    quality_score = self.quality_validator.validate_section(content, section_type, self._get_expected_elements(section_type))

                if quality_score.overall_score >= 0.7:
                    break
                else:
                    if feedback_str:
                        persona_text = persona_text.replace(feedback_str, "")
                    feedback_str = ""
                    for feedback_line in quality_score.feedback:
                        feedback_str += f"- {feedback_line}\n"
                    persona_text = (
                        persona_text +
                        "\nEnsure your response addresses the following concerns:\n" +
                        feedback_str
                    )
                    content, token_usage, cost = await asyncio.to_thread(
                        self.generate_section_content,
...
"""
        ...
        if previous_sections:
            ...
            for section in previous_sections[-2:]:  # Include last 2 sections for context
                # Truncate previous section content to avoid exceeding token limits
                truncated_content = section.content[:500] + "..." if len(section.content) > 500 else section.content
                prompt += f"\n{section.title}:\n"
